...
const BrowseLibrariesSvg = ({ buttonClass }: { buttonClass: string }) => (
...
@Component({
  tag: 'ss-button-browse-libraries',
  styleUrl: '../../styles/ss-button-styles.css',
  shadow: false,
})
export class SsButtonBrowseLibraries {
  /**
   *
   */
    ...
    return (
      <Host>
        <button class={this.unstyled ? '' : 'ss-button-styled'}>
          <BrowseLibrariesSvg buttonClass={this.unstyled ? 'hide-icons' : 'ss-icons'} />
          <slot></slot>
        </button>
      </Host>
    );
